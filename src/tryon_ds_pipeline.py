...
class StableDiffusionXLInpaintPipeline(
    ...
    def __call__(
        ...
        cloth_mask = None,
        pose_img = None,
        #target_pose_img = None,                 # unet encoder에 넣을 densepose
        ...
        **kwargs,
        ...
        target_mask = self.mask_processor.preprocess(
            cloth_mask, height=height, width=width, resize_mode=resize_mode, crops_coords=crops_coords
        )
        cloth = cloth.to(device=device)
        target_mask = target_mask.to(device=device)
        if cloth.shape[1] == 4:
            target_masked = None
        else :
            target_masked = cloth * (target_mask < 0.5)
        ...
        if return_image_latents:
            target_latents, noise, image_latents = target_latents_outputs
        else:
            target_latents, noise = target_latents_outputs
        ...
        target_mask, target_masked_image_latents = self.prepare_mask_latents(
            target_mask,
            target_masked,
            batch_size * num_images_per_prompt,
            height,
        ...
        # target_pose_img = target_pose_img.to(device=device, dtype=prompt_embeds.dtype)

        # target_pose_img = self.vae.encode(target_pose_img).latent_dist.sample()
        # target_pose_img = target_pose_img * self.vae.config.scaling_factor

        # target_pose_img = (
        #         torch.cat([target_pose_img] * 2) if self.do_classifier_free_guidance else target_pose_img
        # )
        pose_img = (
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                cloth = self.scheduler.scale_model_input(target_latents, t)
                ...
                garmentnet_input = torch.cat([cloth, target_mask, target_masked_image_latents], dim=1)
                #cloth = torch.cat([cloth,target_pose_img], dim=1)
                ...
                print(self.unet_encoder.config.addition_embed_type)
                ...
                down,reference_features = self.unet_encoder(garmentnet_input,t, text_embeds_cloth,added_cond_kwargs={"text_embeds": pooled_prompt_embeds_c, "time_ids": add_time_ids},return_dict=False)
